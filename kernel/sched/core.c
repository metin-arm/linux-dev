...
#else
static int __init setup_proxy_exec(char *)
{
...
static inline void activate_blocked_entities(struct rq *,
					     struct task_struct *,
					     int)
{
}

struct task_struct *find_exec_ctx(struct rq *, struct task_struct *p)
{
	return p;
}
...
static inline void proxy_tag_curr(struct rq *, struct task_struct *) { }
...
pick_again:
	...
	if (likely(prev != next)) {
		...
		if (unlikely(prev_not_proxied && next->blocked_donor))
			proxy_tag_curr(rq, next);
